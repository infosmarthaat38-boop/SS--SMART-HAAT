...
/**
 * ProductCard Component - Refined with non-bold Taka symbol and larger original price.
 * Fixed image scaling to prevent cropping.
 */
  ...
  return (
    <>
      <Card className={`group overflow-hidden bg-black border-none transition-all duration-300 rounded-none flex flex-col h-full relative ${isOutOfStock ? 'opacity-70' : ''}`}>
        ...
        {/* IMAGE CONTAINER - Fixed with object-contain to prevent side cropping */}
          ...
          <div className="flex items-center justify-between mt-auto">
            <div className="flex flex-col">
              <span className="font-black text-lg md:text-2xl text-orange-600 tracking-tighter leading-none flex items-baseline">
                {/* SMALL TAKA SYMBOL - LIGHT WEIGHT */}
                <span className="text-[9px] md:text-[11px] font-normal mr-0.5 align-baseline">৳</span>
                {product.price.toLocaleString()}
              </span>
              {product.originalPrice > product.price && (
                <span className="text-white/40 line-through text-[12px] md:text-[14px] font-bold mt-1 flex items-baseline">
                  <span className="text-[8px] md:text-[9px] font-normal mr-0.5">৳</span>{product.originalPrice.toLocaleString()}
                </span>
              )}
            </div>
            
            {product.discountPercentage > 0 && (
              <div className="px-1.5 py-0.5 md:px-2 md:py-1 border border-orange-600/60">
