      ...
      <CardContent className="p-3 flex flex-col flex-grow space-y-2">
        <div className="space-y-1">
          <Link href={`/products/${product.id}`} className="block">
            <h3 className="font-bold text-[10px] line-clamp-2 leading-snug group-hover:text-primary transition-colors h-7 text-foreground uppercase tracking-tight">
              {product.name}
            </h3>
          </Link>
        ...
        <div className="pt-1.5 border-t border-white/5 space-y-1.5">
          <div className="flex flex-col">
            <div className="flex items-center gap-2">
              <span className="font-black text-sm text-primary tracking-tighter">৳{product.price.toFixed(2)}</span>
              <span className="text-[8px] text-muted-foreground line-through font-bold">৳{product.originalPrice.toFixed(2)}</span>
            </div>
          ...
          <Button className="w-full bg-primary text-black hover:bg-white transition-all font-black text-[8px] h-7 rounded-none uppercase shadow-lg shadow-primary/10 px-2">
