...
export default function AdminPanel() {
  ...
  useEffect(() => {
    if (pendingOrders && pendingOrders.length > 0) {
      ...
      // If order is less than 10 seconds old, show a toast
    ...
    <div className="min-h-screen flex flex-col bg-background selection:bg-orange-600/30">
      ...
      <main className="flex-grow container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {stats.map((stat, i) => (
            <Card key={i} className="bg-card border-white/5 rounded-none p-4 md:p-6 hover:border-orange-600/30 transition-all">
              ...
              <div className="flex items-baseline gap-2">
                ...
                <span className={`text-[7px] md:text-[9px] font-black uppercase ${stat.change.includes('PENDING') ? 'text-orange-600 animate-pulse' : 'text-green-500'}`}>
                  {stat.change}
                </span>
              </div>
            </Card>
        ...
        {/* REAL-TIME NOTIFICATION BAR FOR MOBILE/ANY DEVICE */}
        {pendingOrders && pendingOrders.length > 0 && (
          <div className="mb-6 p-4 bg-orange-600/10 border border-orange-600/30 flex items-center justify-between animate-pulse">
            ...
            <Link href="/admin/orders" className="text-[9px] font-black text-orange-600 underline uppercase">VIEW ALL</Link>
          </div>
        )}

        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          {/* LEFT COLUMN: QUICK LINKS & AI */}
          <div className="lg:col-span-4 space-y-6">
            <Card className="bg-card border-white/5 rounded-none">
              <CardHeader className="py-4 px-6 border-b border-white/5">
                <CardTitle className="text-[10px] font-black uppercase tracking-[0.3em] text-orange-600">COMMAND CENTER</CardTitle>
              </CardHeader>
                  ...
                  {quickLinks.map((link, i) => (
                    <Link key={i} href={link.href}>
                      <div className="flex items-center justify-between p-4 hover:bg-white/[0.03] transition-all group border-b border-white/[0.02] last:border-0">
                        ...
                        <ChevronRight className="h-3 w-3 text-white/40 group-hover:text-orange-600 transition-all" />
                      </div>
                    </Link>
                  ))}
                </div>
            ...
            <Card className="bg-card border-white/5 rounded-none overflow-hidden">
              <CardHeader className="py-4 px-6 border-b border-white/5">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-[10px] font-black uppercase tracking-[0.3em] text-orange-600">AI STRATEGIST</CardTitle>
        ...
        {/* BOTTOM ACTIVITY FEED - LIVE LOGS */}
        <div className="mt-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {pendingOrders?.slice(0, 4).map((order, i) => (
              <div key={i} className="bg-card border border-orange-600/20 p-5 flex flex-col gap-3 relative overflow-hidden group hover:border-orange-600 transition-all">
                <div className="absolute top-0 right-0 p-1.5 bg-orange-600">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <Badge className="rounded-none bg-orange-600 text-white text-[7px] border-none font-black h-4 px-1.5">NEW ORDER</Badge>
                </div>
                <div className="space-y-1">
                  <p className="text-[10px] font-black text-white uppercase tracking-tight line-clamp-1">{order.customerName}</p>
                  <p className="text-[9px] font-black text-orange-600 uppercase tracking-widest truncate">{order.productName}</p>
                </div>
              </div>
            )) || (
