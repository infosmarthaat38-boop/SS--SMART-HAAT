...
const generateInvoice = async (order: any, setIsGeneratingPdf: (val: boolean) => void) => {
  ...
  try {
    ...
    doc.text("NAME:", 20, 83);
    doc.setTextColor(0, 0, 0);
    doc.text(toSafePdfText(order.customerName), 45, 83);
    ...
    doc.text("THANK YOU FOR CHOOSING SS SMART HAAT.", 15, 282);
...
export default function AdminOrders() {
  ...
  const handleDeleteOrder = (id: string) => {
    ...
    triggerAlert(
      ...
      "THIS WILL REMOVE ALL DATA FROM THE PERMANENT DATABASE.",
      ...
      <Dialog open={isConfirmOpen} onOpenChange={setIsConfirmOpen}>
        <DialogContent className="bg-black border-orange-500/30 rounded-none max-w-md p-8 fixed left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%] shadow-2xl">
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            <Button variant="outline" onClick={() => setIsConfirmOpen(false)} className="flex-1 rounded-none uppercase text-[10px] font-black h-14">CANCEL</Button>
            <Button disabled={!deliveryCharge} onClick={handleFinalizeConfirmation} className="flex-1 bg-orange-500 hover:bg-orange-600 text-white font-black uppercase text-[10px] rounded-none h-14">CONFIRM</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <AlertDialog open={isAlertOpen} onOpenChange={setIsAlertOpen}>
        <AlertDialogContent className="bg-black border-orange-500/30 rounded-none p-8 max-w-md fixed left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%] shadow-2xl">
          <AlertDialogHeader className="space-y-4">
            <AlertDialogTitle className="text-2xl font-black text-white uppercase tracking-tighter">{alertConfig?.title}</AlertDialogTitle>
            <AlertDialogDescription className="text-[10px] text-muted-foreground uppercase font-black tracking-widest">{alertConfig?.desc}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="mt-8">
            <AlertDialogCancel className="flex-1 rounded-none uppercase text-[10px] h-12">BACK</AlertDialogCancel>
            <AlertDialogAction onClick={() => alertConfig?.action()} className="flex-1 bg-red-600 hover:bg-red-700 text-white font-black uppercase text-[10px] rounded-none h-12">CONFIRM</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
